...
g2 <- df_combined %>% 
  ...
  scale_y_log10() +
  scale_color_brewer("Parameter", palette = "Dark2") +
  xlab("Independent error estimate") +
  ylab("Alternative error model estimate") +
...
```{r}
library(cowplot)
gfinal <- plot_grid(g1, g2, nrow = 2, labels = c("A.", "B."))
ggsave("error_model_comparison.pdf", gfinal, width = 10, height = 6)
```

Residuals! Hmm. Not sure what to plot since all models will have autocorrelated residuals...
```{r}
df_acf <- df %>% 
  pivot_longer(-c(time, current)) %>% 
  mutate(res=current-value) %>% 
  group_by(name) %>% 
  summarise(
    ac=acf(res)$acf
  )
